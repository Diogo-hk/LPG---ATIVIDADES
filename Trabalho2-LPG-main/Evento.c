...
{
    int existe = 1; // Inicializado para 0, porque não encontrou ainda
    for (int i = 0; i < n; i++)
    {
        if (strcmp(v[i].descricao, descricaobusca) == 0)
        { // Verifica se a descrição corresponde
            printf("\n\n");
            printf("Evento encontrado!\n");
            mostra_evento(&v[i]);
            existe = 0; // Evento encontrado, então existe = 1
        }
    }
    if (existe == 1)
    {
